        ...
        <div class="container pt-5 pb-5 pt-md-5 pb-md-10">
            <div class="row gx-lg-8 gx-xl-12 gy-10 mb-5 align-items-center">
                <div class="col-md-10 offset-md-1 offset-lg-0 col-lg-5 text-center text-lg-start order-2 order-lg-0"
                    ...
                    <h1 class="display-1 mb-5 mx-md-n5 mx-lg-0">Penyelenggara</h1>
                    <p class="lead fs-lg mb-7">{{getSetting('app_description')}}</p>
                </div>
                <!-- /column -->
                <div class="col-lg-7" data-cue="slideInDown">
                    <lottie-player
                        src="{{asset('statis/illustrations/planner.json')}}"
    ...
    <section class="wrapper bg-soft-primary" id="pelakuusaha">
        <div class="container py-14 py-md-16">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label>Pencarian penyelenggara</label>
            ...
            <div class="row gx-lg-8 gx-xl-12 gy-10 gy-lg-0 mt-4">
                @if(count($planner) > 0)
                    @foreach($planner as $val)
                        <div class="project item col-md-4 col-xl-4 mission">
                            <figure class="lift rounded mb-6"><a href="{{url('/planner/detail/'.encodeId($val->id))}}">
                                    <img src="{{ getImageOri($val->foto)  }}" alt="" /></a></figure>
                            <div class="project-details d-flex justify-content-center flex-column">
                                <div class="post-header">
                                    <a href="{{url('/planner/detail/'.encodeId($val->id))}}">
                                        <h2 class="post-title h3"> {{$val->name}} </h2>
                                    </a>

                                    <a href="#"> <i class="uil uil-calender"></i> &nbsp; Terdaftar sejak {{ rubah_tanggal_indo($val->created_at) }} </a>
                                </div>
                                <!-- /.post-header -->
                            </div>
                            <!-- /.project-details -->
                    ...
                    {{$planner->links()}}
                @else
                    <h3>Mohon maaf penyelenggara dengan nama {{$keyword}} tidak ditemukan</h3>
                    <a href="{{ url(('/planner'))  }}"> <i class="uil uil-backward"></i> Kembali </a>
                @endif
            </div>
            <!-- /.row -->
        </div>
